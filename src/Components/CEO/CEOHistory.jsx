...
const CEOHistory = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return localStorage.getItem('ceoHistoryTab') || 'all';
  });

  // Save active tab to localStorage when it changes
  useEffect(() => {
    localStorage.setItem('ceoHistoryTab', activeTab);
  }, [activeTab]);
  ...
  const handleDeleteClick = (id) => {
    setDeleteModal({ show: true, reminderId: id, isMultiple: false });
  };
  ...
  const handleDeleteSelected = () => {
    ...
    setDeleteModal({ show: true, reminderId: null, isMultiple: true });
    setShowMenu(false);
  };
  ...
  return (
    <>
      ...
      <div className="p-6 bg-white rounded-xl">
        ...
        {/* History List */}
        <div className="history-scroll-container max-h-[600px] overflow-y-auto space-y-3 pr-2">
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`border-l-[4px] rounded-lg transition-all ${statusColor(item.status)} ${
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start gap-2">
                        <h3 className="font-medium text-gray-800 flex-1">{item.title}</h3>
                        
                        {/* Priority - Top Right */}
                        <span className={`text-xs font-medium px-2 py-1 rounded ${priorityColor(item.priority)}`}>
                          {item.priority}
                        </span>
                      </div>
