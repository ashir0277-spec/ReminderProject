...
const CEOHistory = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('ceoHistoryTab') || 'all');
  const [searchQuery, setSearchQuery] = useState('');

  useEffect(() => { localStorage.setItem('ceoHistoryTab', activeTab); }, [activeTab]);
      ...
      <div className="p-4 lg:p-6 min-h-screen bg-gray-50">
        ...
        {selectionMode && (
          <div className="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-4 animate-fade-in">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setSelectedItems([])} className="text-xs text-blue-600 hover:text-blue-800 font-medium px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors">
                Clear
              </button>
        ...
        {!isSearching && (
          <div className="tabs-container flex gap-2 mb-5 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
            {[
            ...
            <div className="relative bg-white w-full max-w-md rounded-xl shadow-xl animate-slide-up">
              ...
              <div className="modal-scroll overflow-y-auto max-h-[55vh] px-5 py-4 space-y-3 text-sm">
                ...
                <div className="space-y-2.5 pt-1">
                  ...
                  {item.createdAt && (
                    <div className="flex gap-3">
                      <span className="text-gray-400 w-24 shrink-0">Created At</span>
                      <span className="text-gray-800">{item.createdAt?.toDate?.() ? item.createdAt.toDate().toLocaleString() : "N/A"}</span>
                    </div>
                  )}
                  {item.description && (
                    <div className="flex gap-3">
