...
import { IoEllipsisVertical, IoTrashOutline, IoCheckboxOutline, IoClose, IoSearchOutline } from "react-icons/io5";
...
const HRHistory = () => {
  ...
  useEffect(() => {
    const q1 = query(collection(db, "reminders"), where("createdBy", "==", "HR"), orderBy("createdAt", "desc"));
    const q2 = query(collection(db, "reminders"), where("createdBy", "==", "CEO"), orderBy("createdAt", "desc"));
    const q3 = query(collection(db, "reminders"), where("createdBy", "==", "CTO"), orderBy("createdAt", "desc"));
  ...
  const handleDeleteSelected = () => {
    if (selectedItems.length === 0) {
      toast.error("No items selected", { position: "top-center", autoClose: 3000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true });
      return;
    }
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (deleteModal.isMultiple) {
        ...
        selectedItems.forEach(id => { batch.delete(doc(db, "reminders", id)); });
        await batch.commit();
        toast.success(`${selectedItems.length} reminder(s) deleted successfully!`, { position: "top-center", autoClose: 3000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true });
        setSelectedItems([]);
        setSelectionMode(false);
      } else {
        await deleteDoc(doc(db, "reminders", deleteModal.reminderId));
        toast.success("Reminder deleted successfully!", { position: "top-center", autoClose: 3000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true });
      }
    } catch (err) {
      toast.error("Failed to delete reminder(s)", { position: "top-center", autoClose: 3000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true });
      console.error(err);
    }
  ...
  const openDetailsModal = (reminder) => { setDetailsModal({ show: true, reminder }); };
  const closeDetailsModal = () => { setDetailsModal({ show: false, reminder: null }); };
  ...
  const belongsToRole = (reminder, role) => {
    ...
    const assignedToRole = (reminder.assignedTo || '').split(',').map(r => r.trim().toUpperCase()).includes(roleUpper);
  ...
  // ── Search filter ──────────────────────────────────────────────────────────
  const filterBySearch = (reminder) => {
    if (!searchQuery.trim()) return true;
    const q = searchQuery.toLowerCase().trim();
    return (
      (reminder.title || '').toLowerCase().includes(q) ||
      (reminder.description || '').toLowerCase().includes(q) ||
      (reminder.assignedTo || '').toLowerCase().includes(q) ||
      (reminder.createdBy || '').toLowerCase().includes(q)
    );
  };
  ...
  return (
    <>
      <style>{`
        ...
        .tabs-container::-webkit-scrollbar { width: 0px; height: 0px; }
        @keyframes progress { 0% { width: 0%; } 100% { width: 100%; } }
        .animate-progress { animation: progress 1.5s ease-in-out infinite; }
        .history-card { position: relative; }
        .history-card:hover .hover-hint { opacity: 1; }
        .hover-hint { opacity: 0; transition: opacity 0.3s ease; }
      `}</style>
      ...
      <div className="rounded-md min-h-screen border border-[#E2E4E7] p-6">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 bg-gray-50 transition-colors"
          />
          {isSearching && (
            <button
        ...
        {isSearching && (
          <p className="text-sm text-gray-500 mb-4">
            {filteredHistory.length} result{filteredHistory.length !== 1 ? 's' : ''} for <span className="font-medium text-gray-700">"{searchQuery}"</span>
          </p>
        )}
        ...
        {selectionMode && (
          <div className="mb-4 flex justify-end">
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center gap-2 w-fit">
              ...
              <button onClick={exitSelectionMode} className="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                Cancel
              </button>
            </div>
          </div>
          ...
          <div className="tabs-container flex gap-2 mb-5 overflow-x-auto pb-2">
            {[
              ...
              <button
                ...
                onClick={() => { setActiveTab(tab.key); setSelectedItems([]); setSelectionMode(false); }}
              >
                {tab.label} ({tab.count})
              </button>
            ))}
        ...
        {/* History List */}
        <div className="history-scroll-container mt-4 space-y-4 overflow-y-auto max-h-[600px]" style={{scrollbarWidth:'none'}}>
          {filteredHistory.length === 0 ? (
            <div className="text-center py-10 bg-gray-50 rounded-lg">
              {isSearching ? (
                <>
                  <p className="text-gray-500 text-lg">🔍 No results found</p>
                  <p className="text-gray-400 text-sm mt-2">No reminders match "<span className="font-medium">{searchQuery}</span>"</p>
                ...
                <>
                  ...
                  <p className="text-gray-400 text-sm mt-2">
                    {activeTab === 'ceo' ? 'No CEO reminders found' :
                     ...
                     activeTab === 'hr'  ? 'No HR reminders found'  : 'No reminders found'}
                  </p>
                </>
              )}
            </div>
            ...
            filteredHistory.map((item) => {
              ...
              return (
                <div
                  ...
                  onClick={(e) => {
                    if (selectionMode) toggleSelection(item.id, e);
                    else openDetailsModal(item);
                  }}
                >
                  {/* Checkbox - TOP RIGHT in selection mode */}
                  {selectionMode && (
                    <div className="absolute top-3 right-3 z-10" onClick={(e) => toggleSelection(item.id, e)}>
                      <div className={`w-6 h-6 border-2 rounded-md flex items-center justify-center cursor-pointer transition-all ${
                        isSelected ? 'bg-blue-600 border-blue-600 scale-110' : 'bg-white border-gray-300 hover:border-blue-400'
                      }`}>
                        {isSelected && (
                    ...
                    <div className="absolute top-3 right-3 z-10">
                      ...
                      {activePopup === item.id && (
                        <>
                          ...
                          <div className="fixed inset-0 z-40" onClick={(e) => { e.stopPropagation(); setActivePopup(null); }}></div>
                        </>
                      )}
                    </div>
                  )}
                ...
                <div className='pt-2'>
                  ...
                  <div className={`p-4 rounded-lg border ${
                    ...
                    <p className={`text-sm font-semibold mb-1 ${
                      ...
                      detailsModal.reminder.status === 'reject'   ? 'text-red-700' : 'text-orange-700'
                    }`}>
                      {detailsModal.reminder.status === 'approved' ? '✓ Approved' :
                       detailsModal.reminder.status === 'reject'   ? '✗ Rejected' : 'Pending'}
                    </p>
                    {detailsModal.reminder.status === 'approved' && detailsModal.reminder.updatedBy && (
                      <p className='text-xs text-green-600'>Approved by: <span className='font-semibold'>{detailsModal.reminder.updatedBy}</span></p>
                    )}
                    {detailsModal.reminder.status === 'reject' && detailsModal.reminder.rejectedBy && (
                      <p className='text-xs text-red-600'>Rejected by: <span className='font-semibold'>{detailsModal.reminder.rejectedBy}</span></p>
                    )}
                    {detailsModal.reminder.status === 'reject' && detailsModal.reminder.reason && (
                      <div className='mt-2 pt-2 border-t border-red-200'>
                        <p className='text-xs text-red-600'>Reason: <span className='font-semibold'>{detailsModal.reminder.reason}</span></p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 flex items-center justify-center z-[60] p-4">
          <div className="absolute inset-0 bg-black/50" onClick={() => !deleteModal.deleting && setDeleteModal({ show: false, reminderId: null, isMultiple: false, deleting: false })}></div>
